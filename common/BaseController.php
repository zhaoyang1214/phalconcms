...
class BaseController extends Controller {
    ...
    protected function sendJson($responseData, int $status = 10000, int $jsonOptions = null, int $depth = 512){
      ...
      * @return: string 键名
      * @author: ZhaoYang
      * @date: 2018年7月12日 下午11:49:48
      */
    protected static function createCacheKey(string $functionName = '', array $parameters = []) {
        $key = 'data_';
        $classNmae = static::class;
        if (!empty($classNmae)) {
            $key .= md5($classNmae) . '_';
        }
     ...
     * @desc 脚本运行时间
     * @author: ZhaoYang
     * @date: 2018年7月16日 下午10:23:08
     */
    public function runtime(){
        $GLOBALS['script_end_time'] = microtime(true);
        $runTime = number_format($GLOBALS['script_end_time'] - $GLOBALS['script_start_time'], 4);
        return $runTime;
    }
}