...
$di->set('cache', function (array $options = []) {
    ...
    $cacheConfig = $config->services->cache;
    ...
    /* if(isset($config->application->debug->state) && $config->application->debug->state){
        $frontendOption['lifetime'] = 10;
    } */
    if (version_compare(PHALCON_VERSION, '3.2.0', '>')) {
