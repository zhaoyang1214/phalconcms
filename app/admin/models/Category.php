...
class Category extends ModelsCategory {
    ...
    public function rules() {
        return [
            ...
            'pid0' => ['pid', 'callback', '上级栏目选择错误', function($data) {
                if(!isset($data['pid'])) {
                    return false;
                }
                if($data['pid'] == 0) {
            ...
            'category_model_id0' => ['category_model_id', 'callback', '模型选择错误', function($data) {
            ...
            'sequence0' => ['sequence', 'digit', '排序必须为整数'],
            'is_show0' => ['is_show', 'inclusionin', '是否显示选择错误', [0,1]],
            'type0' => ['type', 'inclusionin', '栏目类型选择错误', [1,2]],
            'name0' => ['name', 'stringlength', '栏目名称长度必须大于1位|模型名称必须小于50位', [1, 50]],
            'urlname0' => ['urlname', 'callback', '栏目URL名称已存在', function($data) {
            ...
            'page0' => ['page', 'digit', '分页必须为整数'],
            'content_order0' => ['content_order', 'callback', '内容排序选择错误', function($data) {
                if(!isset($data['content_order'])) {
                    return false;
                }
                return array_key_exists($data['content_order'], self::getContentOrder()) ? true : false;
            }],
            'expand_id0' => ['expand_id', 'callback', '扩展模型选择错误', function($data) {
    ...
    public function getAllowList2() {
        ...
        $result = self::query()
        ->columns('App\Admin\Models\Category.id,App\Admin\Models\Category.pid,App\Admin\Models\Category.category_model_id,App\Admin\Models\Category.type,App\Admin\Models\Category.name,b.category,b.content')
    ...
    public function addAll(array $data) {
        $insertData = Common::arraySlice(['pid', 'category_model_id', 'sequence', 'is_show', 'type', 'subname', 'image', 'category_tpl', 'content_tpl', 'page', 'keywords', 'description', 'seo_content', 'content_order', 'expand_id'], $data);
        $validate = new Validate();
        $message = $validate->addRules(self::getRules(['pid0', 'category_model_id0', 'sequence0', 'is_show0', 'type0', 'page0','content_order0', 'expand_id0'], false))->validate($insertData);
        if (count($message)) {
        ...
        $connection->begin();
        $rules = self::getRules(['name0', 'urlname0']);
        try {
    ...
    public function add(array $data) {
        $insertData = Common::arraySlice(['pid', 'category_model_id', 'name', 'urlname', 'sequence', 'is_show', 'type', 'subname', 'image', 'category_tpl', 'content_tpl', 'page', 'keywords', 'description', 'seo_content', 'content_order', 'expand_id'], $data);
        ...
        $validate = new Validate();
        $message = $validate->addRules(self::getRules(['pid0', 'name0', 'urlname0', 'category_model_id0', 'sequence0', 'is_show0', 'type0', 'page0','content_order0', 'expand_id0'], false))->validate($insertData);
        if (count($message)) {
            return $this->errorMessage($message);
        }
        if(isset($insertData['keywords'])) {
    ...
    public function updateSequenceById(int $id, int $sequence=0) {
        ...
        $message = (new Validate())->addRules(self::getRules(['id0', 'sequence0']))->validate($data);
        if(count($message)) {
            return $this->errorMessage($message);
        }
    ...
    public function edit(array $data) {
        $updateData = Common::arraySlice(['id', 'pid', 'name', 'urlname', 'sequence', 'is_show', 'type', 'subname', 'image', 'category_tpl', 'content_tpl', 'page', 'keywords', 'description', 'seo_content', 'content_order', 'expand_id'], $data);
        ...
        $validate = new Validate();
        $message = $validate->addRules(self::getRules(['id0', 'pid0', 'name0', 'urlname0', 'sequence0', 'is_show0', 'type0', 'page0','content_order0', 'expand_id0'], false))->validate($updateData);
        if (count($message)) {
            return $this->errorMessage($message);
        }
